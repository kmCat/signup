...
class MainPage(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if not name:
            error_name = "Please enter a valid Username".format(username)
            errname_escaped = cgi.escape(error_name, quote=True)

        if username == "":
            error_noname = "Please enter a valid username.".format(username)
            errname_escaped = cgi.escape(error_noname, quote=True)

        if not pwd:
            error_pwd = "Please enter a valid password.".format(password)
            errpwd_escaped = cgi.escape(error_pwd, quote=True)

        if password != verify:
            error_match = "Please re-enter you password.".format(verify)
            errmatch_escaped = cgi.escape(error_match, quote=True)

        if not mail:
            error_email = "Please enter a valid email address".format(email)
